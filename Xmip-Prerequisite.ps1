...
.DESCRIPTION
    ...
    Reporting is the default. -Install acts, the same rule as Xmip-Estate.
...
.EXAMPLE
    ...
    Xmip-Prerequisite -Role developer

.EXAMPLE
    Xmip-Prerequisite -Role developer -Install -WhatIf
#>
function Xmip-Prerequisite {
    ...
    param(
        ...
        [string] $ManifestPath = (Join-Path $PSScriptRoot 'prerequisite.toml'),
    ...
    function Get-XmipReportedVersion([string] $Text) {
        # pwsh   -> "PowerShell 7.6.5"
        # dotnet -> "11.0.100-preview.3.26xxx"   (prerelease suffix dropped)
        # git    -> "git version 2.45.1"
        # rustup -> "rustup 1.27.1 (abc 2026-01-01)"
    ...
    function Test-XmipFloor([string] $Name, $Found, [string] $Minimum) {
        # ADR-0021 is enforced here, not stated in a comment. A prerequisite that
        # declares a minimum and reports a lower version is a failure, not a
        # warning, and not something the caller can miss.
        if (-not $Minimum) { return $true }
        $actual = Get-XmipReportedVersion $Found
        if (-not $actual) {
            Write-Warning "UNVERIFIABLE: $name reports '$Found', which carries no version to compare against $Minimum."
            return $true
        }
        if ($actual -ge [version]$Minimum) { return $true }
        return $false
    }
    ...
    foreach ($name in (Get-TomlKey $prerequisites)) {
        ...
        if ($found) {
            if (Test-XmipFloor $name $found $minimum) {
        ...
        if ([string](Get-TomlValue $item 'source' '') -eq 'psgallery') {
            ...
            if ($good) {
                Write-Host "PRESENT: $name  ($(($present.Version | Sort-Object -Descending | Select-Object -First 1)))"
                Record $name 'present' 'psgallery'
            }
            elseif (-not $Install) {
                $why = if ($present) { "OUTDATED: $name is $($present.Version | Sort-Object -Descending | Select-Object -First 1)" } else { "MISSING: $name" }
            ...
            elseif ($PSCmdlet.ShouldProcess($id, 'Install from the PowerShell Gallery')) {
                ...
                # Pester 3 whose publisher differs from the gallery's.
                Install-Module -Name $id -Scope CurrentUser -Force -AcceptLicense -SkipPublisherCheck
                Record $name 'installed' 'psgallery'
            }
