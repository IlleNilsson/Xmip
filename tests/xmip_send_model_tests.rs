...
#[test]
fn retryable_failure_retries_same_send_location_and_does_not_fail_over() {
    let (mut interchange, message_id) = source_interchange();
    let send_port = SendPort::new(
        "sendPort:orders-out",
        vec![
            SendLocation::new("location-a", "https://a.example/orders", 2),
    ...
    assert_eq!(result.location_results[0].location_name, "location-a");
    assert_eq!(result.location_results[0].attempts, 3);
    assert!(result.error.expect("error expected").contains("retryable retries"));
}

#[test]
fn all_send_locations_failed_by_non_retryable_errors_returns_error_status() {
    let (mut interchange, message_id) = source_interchange();
    let send_port = SendPort::new(
        "sendPort:orders-out",
        vec![
...
fn send_side_promotion_and_transform_create_new_messages() {
    ...
    assert!(interchange
        .audit
        .iter()
        .any(|entry| entry.outcome == "send-side promotion"));
    assert!(interchange
        .audit
        .iter()
        .any(|entry| entry.outcome == "send-side transform"));
}
...
fn send_port_group_executes_independent_send_ports() {
    ...
    let port_b = SendPort::new(
        "sendPort:webhook",
        vec![SendLocation::new(
            "webhook-location",
            "https://webhook.example/orders",
            0,
        )],
    )
    .expect("send port should be valid");
