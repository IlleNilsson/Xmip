# Xmip Architecture Specification v1.2
...
- Contract implication from Receive Configuration, the incoming Stream and Message Context;
- evaluation coordination;
- Contract descriptors and references;
- common results and diagnostics.

`xmip-contract` does not own representation parsing, serialization or Path execution. Representation implementations materialize or serialize message content. Path implementations address materialized content. Contract implementations evaluate the applicable Stream, Message or materialized structure without absorbing those other responsibilities.
...
Examples: `xmip-receive`, `xmip-send`, `xmip-transport`, `xmip-logic`, `xmip-prepare`, `xmip-identify`, `xmip-authenticate`, `xmip-authorize`, `xmip-contract`, `xmip-path`, `xmip-assign`, `xmip-transform`, `xmip-route` and `xmip-process`.
...
`handler` is not a repository-name segment. It describes a runtime module role.
...
## 14. Contract, representation, Path, Transport and Logic boundaries

The repository model follows these rules:

- `xmip-receive` owns Receive Ports, Receive Locations and inbound orchestration.
- `xmip-send` owns Send Ports, Send Groups, Send Locations and outbound orchestration.
- `xmip-transport` owns direction-neutral transport contracts.
- `xmip-transport-<technology>` repositories implement transport technologies and declare receive, send or bidirectional capability.
- `xmip-message-<representation>` repositories implement serialization, deserialization and materialization.
- `xmip-contract-<technology>` repositories implement Contract implication or evaluation technology.
- `xmip-path-<language>` repositories implement addressing languages.
- `xmip-logic-<technology>` repositories implement method and operation semantics such as SOAP, HTTP API and gRPC.
- Credential and identity presentation is security functionality, not a transport technology.
- Technology-to-technology dependencies, such as HTTP on TCP or SOAP on HTTP, must be declared explicitly in the manifest.

Every repository has one primary Rust crate whose name matches the repository name unless an accepted architecture decision defines an exception.
