...
| `Get-XmipTestResult`, `Get-XmipHistory` | What a run reports, now and over time. `Get-XmipTestResult` reads the snapshot through the operator module's `Xmip.Surface` and ranks by the runtime's order, as every surface does; the module is built into the session's own directory on first use, and the runtime (`cargo build` in `module/platform/runtime`) must be built. |
