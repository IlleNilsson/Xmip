...
**Still to do in phase B:** row r (headers at arrival); the copies the 2026-09-25 wave found and left: two 64 MiB body ceilings (`transport::wire::MAX_BODY`, `net::http::MAX_BODY`), two TCP connects (`transport::socket::connect_tcp`, `net::http::connect`), the scheme aliases `as_http` in as2, as4 and webdav and msmq's `queue_url`, websocket reading its own `ws://` URL, `aws::query::parameters` decoding a form again, redpanda's own 5xx rule, and as2 and as4 serving their own accept because `serve_one` hides the peer address; dead code, for the owner to rule on.

**Done 2026-09-25:** the `Loopback` trait learned `exchanges_in_order` and `FarEnd::datagram`, and 16 `round` and 26 `unblock` overrides are gone (ADR-0051); HTTP/1.1 is one codec, `net::http`, with `net::Endpoint` and `net::head`, and http keeps TLS, serving and status (ADR-0044); the CAN pair is one `can_bus::loopback::Session`; `xmip-core-library-ssh` holds RFC 4251's types (ADR-0050); a header is `<protocol>.header.<name>` with case folded per protocol (ADR-0046).
