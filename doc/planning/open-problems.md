...
| n | Promotion at arrival | `runtime/src/arrival.rs` routes with `Promoted::from_context` rather than `route::promote`, so in a running node bytes under a bare `X` decline as nothing promoted instead of being refused, and no prefixed property (`header:`, `party:`, ...) is read at arrival | arrival promotes the names its subscriptions use through `route::promote` and the loaded sources (found 2026-09-24 fixing row h) |
...
| q | Where a node sits in a scope | `ScopeTree.Node` takes segment 0, so the Monitor's rows name the cluster as a record's node, and the prompt's `SegmentRender.At` reads the literal `node` segment itself: the scope's shape known in two surfaces (found 2026-09-25) | the node a scope belongs to comes from `observe` through the runtime, as the topology's words do since the same day |
...
**Done 2026-09-24, phase B's first wave.** `xmip-core-library-codec` holds the one hex, base64 and base64url, SHA-1, parameterised CRC, byte cursor and writer, varint with zig-zag, civil calendar, character reader, TOML string and SQL delimiter, and every copy is gone (transport's cursor, hex and CRC among them). `xmip-core-transport-aws` and `xmip-core-transport-azure` exist and hold SigV4, the Query API, JSON 1.1, SAS and Shared Key; http holds HTTP (ADR-0044, amendment 2026-09-24). `rabbitmq` rides the one AMQP 0-9-1 in `amqp`; the three SQL row archives are one `archive::sql::SqlArchive<D>`. Identity reads Kerberos, NTLM type 3, JOSE keys, the clock and window, and the names crossing the gates each in one place, and the 2026-09-22 `ber`/`der` shims are gone (ADR-0050, amendment 2026-09-24). Found by it and fixed: hex took a sign, AS2's base64 a `=` anywhere, SAML and asn1 30 February, the MySQL and PostgreSQL far ends a doubled delimiter as an end, and jwt, oidc and oauth2 a token at its `exp`.
...
**Still to do in phase B:** the `Loopback` trait learns an in-order declaration and an address-aware default `unblock`, and the 42 overrides go (the owner, 2026-09-24); http's own exchange calls `net::http`; `xmip-core-library-ssh` holds RFC 4251's types for sftp and ssh-key (the owner, 2026-09-24); a header a transport writes is `<protocol>.header.<name>`, built once in `context::property`, and route/header names the protocol (the owner, 2026-09-24); j1939's own two-node session; dead code, for the owner to rule on.
