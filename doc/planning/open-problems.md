...
**Still to do in phase B**, found by the first wave: loopback rigs; dns and mdns; CANopen and EtherCAT SDO; webdav's HTTP codec and three MIME writers; percent-encoding four times; SSH string and mpint twice; `uri_path` four times; whether `archive/sqlite` joins `SqlArchive`; the hashed secret store in api-key and bearer; `pub use` re-exports (`is_text` in three SQL crates, `identify::Presented` in authenticate); the three HTTP clients, MAC notation and CIDR into `xmip-core-library-net`; the runtime's own byte reader in `loaded_contract.rs`; five cross-language copies (the rollup rule, counted words, topology words, a node's capability evidence, the snapshot schema); SMB's third NTLM reader and where names a transport writes for identify live; dead code.
