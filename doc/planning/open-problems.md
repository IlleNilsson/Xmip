...
| i | A node's capability | parsed three ways: Playground and PowerShell refuse an unknown word, the surface drops it silently and is case-sensitive | **Resolved 2026-09-24:** `node::Stage::declared` is the one parse, its words exact lowercase only (the owner, 2026-09-24): `Send` or `RECEIVE` is refused by name like any unknown word, never dropped; the Playground reads `--can`, `--nodes` and published records through it and its own `Stage` is gone; PowerShell and `Xmip.Surface`, which reach no Rust for this, follow the same rule, and `test/XmipTest.Test.ps1` holds the words, the case rule and the refusal equal (ADR-0056, amendment 2026-09-24). Placement is row o |
...
| k | Scope containment | `observe` compares text, `ScopeTree` strips scheme and authority first | **Resolved 2026-09-24:** `observe::Scope` reads a scope by its path as `ScopeTree` does, and the snapshot and activity log decide containment only through it; the two writers are held to one file, `observe/src/scope-vector.toml`, which a test in each language reads, because a surface loads no Rust library (the owner; ADR-0052, amendment 2026-09-24) |
